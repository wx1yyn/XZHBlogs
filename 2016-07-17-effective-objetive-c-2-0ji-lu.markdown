...
这样的话，其实就可以组合一些不同类型的对象，来实现一个类的`多继承`的效果。
...
之前，还记录过一种NSProxy然后再加上如上方法实现屏蔽内部版本实现类，其实大体上都差不多，主要是在消息转发阶段，将消息转发给其他能够处理消息的对象。
...
- (1) 调用 `当前NSObject对象` 的 `doesNotRecognizeSelector:`函数实现

- (2) 而在`doesNotRecognizeSelector:`默认实现中做两件事:
	- 结束掉消息转发过程
...
- (1) 创建一个要拦截方法实现目标类的一个`子类`，然后重写要拦截实现的函数
...
- (2) 使用`class_addMethod()、class_replaceMethod()、method_exchangeImplementations()` 交换两个`objc_method`实例的SEL指向
...
- (3) 强制进入消息转发（JSPatch就是这么做的）

代码比较复杂就不贴了，其主要思路就是让我们调用的函数实现与系统的消息转发函数实现进行交换，然后再交换系统的消息转发函数实现，从而拿到所有的方法执行参数。

***

###一个Objective-C类、一个Objective-C类的对象

每一个`Objective-C类`生成的的`对象`，都是指向指向某一块内内存数据的`指针`，所以在声明变量时需要使用`*`来表示指针类型。

而且在Objective-C语言在宏规定一个Objective-C类的对象只能创建在`堆上`，而不能够创建在`线程栈上`，所以不会看到如下这种创建对象的语法:
...
- (1) 类本身 >>> 我们直接操作的类，用来存放`对象`的:
	- 所有的函数实现（属性setter/getter函数实现）
	- 实例变量

- (2) Meta Class 元类 >>> 并非我们直接可以操作的类，存放`类`的所有的`函数实现` 


> 小结下在 objc_class 结构体中两个非常重要的指针:

- (1) `Class *isa` >>> `isa`指针，决定当前对象所属哪一个类型

- (2) `Class *super_class` >>> `super_class`指针，决定当前对象继承自哪一个类型

> 这两个类都对应一个`唯一`的 **objc_class**结构体实例:

即`[类/Meta类 class]` 返回永远是一个单例.
...
注意，`<objc/runtime.h>`中并没有提供函数直接获取Meta Class 元类的`objc_class`实例，只有如下关于Meta Class 元类的函数:
...
- (1) 对象方法实现，查找OC类本身的`objc_class`实例
- (2) 类方法实现，查找OC类本身的`objc_class`实例的isa指针指向的另外一个`objc_class`实例
...
> 拷贝一个对象，就是创建出这个对象类型的另一个新的对象出来，并完成数据复制。
...
可以看到，得到的每一个对象都是新的对象，但是其数据都是一样的，这就是对象拷贝的作用。
...
我们可以让Person类去实现mutableCopyWithZone:所在的`NSMutableCopying协议`，但是有没有这个必要？

没有的。因为 NSCopying协议 与 NSMutableCopying协议 的区别是:

- (1) NSMutableCopying协议 >>> 指针一个类对象只有一种 `可变` 情况
- (2) NSCopying协议 >>> 指针一个类对象只有一种 `不可变` 情况

对于上述的Person类，其实并没有必要分为可变与不可变。但比如NSArray与NSMutableArray、NSURLRequest与NSMutableURLRequest，这些类就分为可变与不可变两个版本:
...
从上面分为可变与不可变版本的类型来看，可以总结出设计这种类型时候的一些原则吧。

针对如上这些分为 可变与不可变 两个版本时，就需要同时实现` NSCopying协议 与 NSMutableCopying协议`，并让外界来选择:

- (1) 调用`copy`返回不可变对象
- (2) 调用`mutableCopy`返回可变对象

我看到很多文章说: `copy就是浅拷贝，mutableCopy就是深拷贝`，这真的是错的十万八千里了，严重误导初学者的。

还有一些虽然正确但是不是太好的设计说，在`copy或mutableCopy`方法实现中，采用`浅拷贝或深拷贝`的拷贝方式去选择性`实现对象拷贝`。针对这种说法是正确，做法也是正确的，但是在此书中建议开发者不要这么做，是有原因的后面再说。

> 比如我们经常对一个数组对象进行copy或mutableCopy，不知道大家注意过数组内子对象的区别没有，下面例子测试下数组对象在不同情况下的拷贝后，其内部子对象的变化。
...
- (1) mutableCopy后得到的数组对象类型是`__NSArrayM`，实际就是苹果内部是有的真正的可变数组对象类型

- (2) 不管 copy出来的数组对象，还是mutableCopy出来的数组对象，其内部子对象还是`原来数组对象中的子对象`
	- 也即是说直接是有指针`指向`原来数组中对象`子对象`
	- `并不是`对原数组内子对象，进行拷贝出新的对象


> 而关于对象的 浅拷贝 与 深拷贝，其关键区别也正是，是否对当前拷贝对象内部的 所有子对象 继续进行拷贝生成新的子对象:
...
那么，将上述数组拷贝的例子改成`深拷贝`后测试一下，虽然Foundation库中基本上所有的类对象拷贝实现都是采用的`浅拷贝`但是对于`容器类型对象`还是提供了可以直接`深拷贝`的函数api:
...
从如上例子也可以总结出 `copy、mutableCopy` 和 `浅拷贝、深拷贝`的关系了:
...
所以说，有些文章说copy就是浅拷贝，mutableCopy就是深拷贝，真的是错的十万八千里了，太离谱了。

下面再看个稍微实用点的例子，也是来自书上的：

> 向外提供不可变数组对象，但向外提供函数api间接操作内部可变数组对象，而内部持有的是不可变数组对象。先看使用浅拷贝实现的代码:
...
- (1) 首先，可以像NSCopying协议和NSMutableCopying协议这样，同样定义出两个版本的深拷贝对象协议，一个是深拷贝可变对象，另一个是深拷贝不可变对象
...
对于前面的Person同样只需要实现NSDeepObjectCopying协议，即只存在不可变的版本.
...
深拷贝出来的对象，其内部子对象继续会使用拷贝，所以就要求所有的类型都需要实现 `NSCopying协议 或 NSMutableCopying`。

所以归根结底，`浅拷贝`是实现 `深拷贝` 的基础，当向一个对象发送`深拷贝`消息时的执行步骤:
...
###`MRC`下对象的释放机制是根据对象的`引用计数器 retainCount`来决定对象是否释放，`ARC`下是根据是否存在strong强指针指向来决定对象是否释放，但实际上ARC还是使用的`引用计数器retainCount`的原理

> MRC/ARC下，如下4个关键的核心概念

- (1) 自己创建的对象，由自己持有

`new/alloc/copy/mutableCopy`开头的方法返回的对象

- (2) 别人创建的对象，我也能够去持有

```
MRC下: id obj = [别人创建的对象 retain];
ARC下: id __strong *pointer = 别人创建的对象;
```

- (3) 自己持有的对象，在不需要使用时候对其释放
...
> ACR下对象的持有与释放 和 MRC下对象的持有与释放

- (1) ARC下，是通过 `strong强引用类型 指针` 
	- 持有，增加一个strong指针
	- 释放，减少一个strong指针，即 `strong指针变量 = nil`;

- (2) MRC下，是通过 NSObject提供的函数api
...
> ARC下对象内存管理策略，对实例变量指针提供了如下几种对象指针修饰符:
...
```objc
//2. 弱引用类型，不会让 retainCount + 1，所以就不能持有一个对象
id __weak obj = [NSObject new];//创建之后立马就被废弃了
```

看下 strong与weak指针修饰符的区别:
...
从输出结果和retainCount变化过程分析，一旦对象的retainCount（对象的strong引用指针个数）变为0，就会立马被废弃掉，而废弃这个对象的过程是`同步`执行的，并非异步后台执行。
...
> 第一种、delegate指针变量必须使用`weak`来修饰，否则会造成循环强引用，从而导致内存泄露
...
解决方法二、主动让持有delegate的对象对执行`delegate = nil;`释放对delegate对象的持有。

> 第二种、对象Block对象引用自己或别人，也会造成循环强引用，从而导致内存泄露
...
- (void)apiCall:(void (^)(id entity))block {
    ...
    // 设置Request对象的Block对象，并将当前ViewModel对象的指针self传入Block对象去持有
    _req.block = ^() {
...
```
2016-07-26 23:51:19.102 Demos[8410:76457] 当前创建的Request对象: 0x7ff3724c7eb0
2016-07-26 23:51:19.103 Demos[8410:76457] 开始网络请求
```

但是并没有看到任何的关于ViewModel对象dealloc信息 和 Request对象dealloc信息。那么说明，此时发生了内存泄漏。

分析下ViewModel对象与Request对象之间的持有关系:

```
1. ViewModel对象通过实例变量 _req 持有 Request对象
2. Request对象通过block变量持有的Block对象，而Block对象又持有了ViewModel对象
```

那么这样就造成了ViewModel对象与Request对象之间互相进行持有，导致ViewModel对象与Request对象的retainCount都是等于2，从而都不会减为0就不会被废弃。

解决方法一、类似上面delegate的思路，让其中一方`主动`解除对另外一方的strong引用。

改动地方一、给Request添加方法api，主动解除strong引用
...
- (void)start {
    ...
    dispatch_async(dispatch_get_main_queue(), ^() {
    	...
    	// 执行完回调之后废弃掉Block对象，从而解决对外部对象的持有
		[self clearBlocks];
    });
}
...
这是Request对象通过Block实例变量引用了另外一个ViewModel对象造成的循环引用，那么还有一种就是Request对象通过Block实例变量引用Request对象自己，也会造成循环引用。造成的原因和上述产生原因是一致的，解决的办发也是一样的。
...
如上代码一样会产生循环强引用，而不同的是Request对象自己引用自己。解决办法和前面一样的，主动将Block对象释放废弃，从而清除Block对象对Request对象持有。
...
比如，YYCache就会让对象的废弃处于一个`子线程`上去执行，那么也就意味着dealloc方法是在`子线程`上完成执行，即在`子线程`上修改对象的retainCount改为0
...
> 但如果Block捕获的是 `对象类型` 的指针，就会自动对指针指向的对象 `发送retain消息` ，也就是会去`持有`指针指向的对象，让这个对象的retainCount++ 。
...
####下面测试一下Block对 `外部变量`、 `外部对象` 分别是如何捕获的
...
对于OC对象，拷贝的是`地址`，而不是`数据`。也就是说Block对象会对外部对象形成一个`strong指向`，即让外部对象retainCount++。

- (3) 如果Block需要修改外部的普通基本类型的变量值
...
可以得到dispatch sync如下几点:

- (1) 多线程任务都是按照`顺序`执行完毕
- (2) `dispatch_sync()`只有当一个block任务执行完毕之后，才会去执行另外一个block任务
- (3) 所有的Block最终执行线程换成了`子线程`
...
###CoreFoundation中的CFMutableDictionaryRef使用

- 第一步，构造CFDictionaryRetainCallBack函数指针执行的类型
...
```
const void* CustomerRetainCallback(CFAllocatorRef allocator, const void *value) {
...
- 第二步、构造CFDictionaryReleaseCallBack函数指针执行的类型
...
```
void CustomerReleaseCallback(CFAllocatorRef allocator, const void *value) {
...
- 第三步、开始创建CFDictionaryKeyCallBacks结构体实例、CFDictionaryValueCallBacks结构体实例
...
- (void)test {
	...
	CFDictionaryKeyCallBacks keyCallbacks = {
        0,//默认版本为0，目前编写程序时都写0
        CustomerRetainCallback,//key被retain的回调
        CustomerReleaseCallback,//key被release的回调
        NULL,//CFDictionaryCopyDescriptionCallBack赋值NULL，采用默认实现
        CFEqual,//切换到 -[NSObject isEqual]
        CFHash,//切换到 -[NSObjecti hash]
    };
   
   //2. 对存入字典value的回调设置
    CFDictionaryValueCallBacks valueCallbacks = {
        0,
        CustomerRetainCallback,//value被retain的回调
        CustomerReleaseCallback,//value被release的回调
        NULL,//CFDictionaryCopyDescriptionCallBack赋值NULL，采用默认实现
        CFEqual,//切换到 -[NSObjecti hash]
    };
    
    //3. 创建字典
    CFMutableDictionaryRef dic = CFDictionaryCreateMutable(NULL,//默认CFAllocatorRef
                                                           10,//最初的字典长度，但是不限制大小
                                                           &keyCallbacks,//key的回调:retain与release
                                                           &valueCallbacks//value的回调:retain与release
                                                           );

                                                           
	//4. 使用字典 
	CFDictionarySetValue(dic, (__bridge const void *)([self class]), (__bridge const void *)(self));
...
全部使用系统默认实现的callback函数实现即可。


###CFMutableDictionaryRef遍历

首先看NSMutableDictionary构建、遍历
...
```c
void MyCFDictionaryApplierFunction(const void *key, const void *value, void *context);
```
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    CFMutableDictionaryRef _dic = CFDictionaryCreateMutable(CFAllocatorGetDefault(), 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
    CFDictionarySetValue(_dic, @"key1", @"value1");
    CFDictionarySetValue(_dic, @"key2", @"value2");
    CFDictionarySetValue(_dic, @"key3", @"value3");
    
    struct MyContext ctx = {0};
    ctx.object = (__bridge void *)(self);
    ctx.otherArgs = @"其他参数";
    
    CFDictionaryApplyFunction(_dic, MyCFDictionaryApplierFunction, &ctx);
}
```

```c
void MyCFDictionaryApplierFunction(const void *key, const void *value, void *context) {
    struct MyContext *ctx = context;
    NSLog(@"key = %@, value = %@, context->object = %@, context->otherArgs = %@", key, value, ctx->object, ctx->otherArgs);
}
```
...
```
2016-10-12 23:20:45.387 demo[2080:27596] key = key1, value = value1, context->object = <ViewController: 0x7fc273742210>, context->otherArgs = 其他参数
2016-10-12 23:20:45.387 demo[2080:27596] key = key3, value = value3, context->object = <ViewController: 0x7fc273742210>, context->otherArgs = 其他参数
2016-10-12 23:20:45.388 demo[2080:27596] key = key2, value = value2, context->object = <ViewController: 0x7fc273742210>, context->otherArgs = 其他参数
```

可以看到，使用CFMutableDictionaryRef进行遍历的时候，可以给callback函数传入一个自定义的struct结构体实例，通常称作`XXXContext`。用这个struct结构体实例可以打包一些`参数、oc对象`传递给callback函数中使用。

###对传入key的处理

- 对key执行`copy`拷贝，得到一个新的，所以key必须`实现NSCopying协议`，否则会报错
- 然后对key进行retain
